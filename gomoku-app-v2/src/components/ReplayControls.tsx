...
const ReplayControls = ({ moveCount, currentMove, setCurrentMove, isPlaying, setIsPlaying, onWhatIf }: ReplayControlsProps) => {
    ...
    return (
        <div className="w-full max-w-lg mt-4 p-3 bg-gray-700 rounded-lg flex items-center gap-4 text-white">
            ...
            <button onClick={() => setCurrentMove(p => Math.max(0, p - 1))} title={t('Previous')} className="btn-hover-scale">‹</button>
            <button onClick={() => setIsPlaying(p => !p)} className="w-20 px-2 py-1 bg-blue-600 rounded btn-hover-scale">{isPlaying ? t('Pause') : t('Play')}</button>
            <button onClick={() => setCurrentMove(p => Math.min(moveCount - 1, p + 1))} title={t('Next')} className="btn-hover-scale">›</button>
